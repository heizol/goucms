...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
   ...
   // 今日揭晓商品
	...
	public function userlogin(){	
		...
		$password = md5(safe_replace($_POST['password']));
	...
	public function userMobile(){	
		...
		$time=time();	
$decode=_encrypt($this->segment(6),"DECODE");
	...
	public function mobileregsn(){
		...
		$member=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `mobile` = '$mobile' LIMIT 1");
			...
			$this->db->Query("UPDATE `@#_member` SET mobilecode='1',`score`=`score`+'$fili_cfg[f_phonecode]',`jingyan`=`jingyan`+'$fili_cfg[z_phonecode]' where `uid`='$member[uid]'");
			$time=time();
		//福分、经验添加
		if($fili_cfg['f_visituser']){
			$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[yaoqing]','1','福分','邀请好友奖励','$fili_cfg[f_visituser]','$time')");
			//邀请人送福分	
			$this->db->Query("UPDATE `@#_member` SET `score`=`score`+'$fili_cfg[f_visituser]',`jingyan`=`jingyan`+'$fili_cfg[z_visituser]' where uid='$member[yaoqing]'");
		}	
	...
	public function getUserBuyList(){
	   $type=intval($this->segment(4));
	   $uid=intval($this->segment(5));
	   $FIdx=intval($this->segment(6));
