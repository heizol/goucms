...
final class template_cache {
	...
	*/
	...
	private function template_parse(){	
		...
		$this->content = preg_replace ( "/\{$stag:$foreach\s+(\S+)\s+(\S+)\}/", "<?php \$ln=1;if(is_array(\\1)) foreach(\\1 AS \\2): ?>",$this->content);										
		$this->content = preg_replace ( "/\{$stag:$foreach\s+(\S+)\s+(\S+)\s+(\S+)\}/", "<?php \$ln=1; if(is_array(\\1)) foreach(\\1 AS \\2 => \\3): ?>",$this->content);										
